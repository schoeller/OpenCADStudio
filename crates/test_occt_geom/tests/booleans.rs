...
//!
//! Tests load committed golden files.  A golden file whose `expected` object
//! contains only `null` values means the recipe is expected to produce no
//! solid.
...
#[test]
fn boolean_goldens() {
    for case in cadrum::boolean_cases() {
